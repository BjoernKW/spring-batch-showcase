...
public class BatchConfiguration {
  ...
  public JdbcCursorItemReader<Order> databaseReader(
    ...
    reader.setSql(
        "SELECT id, customer_id, product_name, quantity, unit_price, order_date, status FROM orders WHERE status = 'PENDING'");
    reader.setRowMapper(new BeanPropertyRowMapper<>(Order.class));
    return reader;
  }

  @Bean
  public JdbcBatchItemWriter<ProcessedOrder> databaseWriter(
      @Qualifier("postgresqlDataSource") DataSource dataSource
  ) {
    JdbcBatchItemWriter<ProcessedOrder> writer = new JdbcBatchItemWriter<>();
    writer.setDataSource(dataSource);
    writer.setSql(
        "INSERT INTO processed_orders (original_order_id, customer_id, product_name, quantity, " +
